...
import net.Indyuce.mmoitems.version.ReflectionUtils;
import net.Indyuce.mmoitems.version.ReflectionUtils.PackageType;
...
public enum ParticleEffect {
	...
	public void display(float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center) {
		if (!isSupported()) {
			throwError("You cannot use this particle.");
			return;
		}
		...
		new ParticlePacket(this, offsetX, offsetY, offsetZ, speed, amount, null).sendTo(center, 100);
	}
	...
	public void display(Vector direction, float speed, Location center) {
		...
		if (!hasProperty(ParticleProperty.DIRECTIONAL)) {
			throwError("This particle is not directional, please use another one.");
			return;
		}
		if (hasProperty(ParticleProperty.REQUIRES_WATER) && !isWater(center)) {
			throwError("You cannot use this particle.");
		...
		new ParticlePacket(this, direction, speed, null).sendTo(center, 100);
	}
	...
	public void display(ParticleColor color, Location center) {
		...
		if (!hasProperty(ParticleProperty.COLORABLE)) {
			throwError("This particle is not colorable, please use another one or remove the color.");
			return;
		}
		if (!isColorCorrect(this, color)) {
			throwError("An error occured reading the particle color.");
			return;
		}
		new ParticlePacket(this, color).sendTo(center, 100);
	}
	...
	public void display(ParticleData data, float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center) {
		...
		if (!isDataCorrect(this, data)) {
			throwError("An error occured reading the particle data.");
			return;
		}
		new ParticlePacket(this, offsetX, offsetY, offsetZ, speed, amount, data).sendTo(center, 100);
	}
	...
	public void display(ParticleData data, Vector direction, float speed, Location center, double range) {
		...
		new ParticlePacket(this, direction, speed, data).sendTo(center, range);
	}

	private long lastError = 0l;

	private void throwError(String message) {
		if (lastError + 6000 > System.currentTimeMillis())
			return;

		lastError = System.currentTimeMillis();
		MMOItems.plugin.getLogger().log(Level.SEVERE, "Item Particles: " + name() + ": " + message);
	}
	...
	public static final class ParticlePacket {
		 ...
		 * @see #initialize()
		 */
		public ParticlePacket(ParticleEffect effect, float offsetX, float offsetY, float offsetZ, float speed, int amount, ParticleData data) throws IllegalArgumentException {
			initialize();
			if (speed < 0) {
		 ...
		 *      boolean, ParticleData)
		 */
		public ParticlePacket(ParticleEffect effect, Vector direction, float speed, ParticleData data) throws IllegalArgumentException {
		...
		public ParticlePacket(ParticleEffect effect, ParticleColor color) {
			this(effect, color.getValueX(), color.getValueY(), color.getValueZ(), 1, 0, null);
		...
		private void initializePacket(Location center) throws PacketInstantiationException {
			...
			try {
				...
				if (version < 8) {
					...
					ReflectionUtils.setValue(packet, true, "j", false);
					if (data != null) {
